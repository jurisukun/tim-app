...
function AddClient() {
  const [isAddNotesVisible, setIsAddNotesVisible] = useState(true);
  ...
  const data=[{label:"Add Client",value:"ADD CLIENT",desc: <div>
                <div>
              ...
              </div>},{label:"Primary Intake",value:"PRIMARY INTAKE",desc:<div className=" lg:h-[60vh] md:h-[50vh] h-[70vh] w-full overflow-y-auto ">
              <div>
                <div>
                  <Card className="mt-6 p-4">
            ...
            </div>},{label:"Service Details",value:"SERVICE DETAILS",desc:<div>
    ...
    <div className="w-full fixed top-0 left-0 h-screen flex justify-center items-center bg-black/70">
      ...
      <TabsHeader>
        {data.map(({ label, value }) => (
          <Tab key={value} value={value} className="w-">
            {label}
          </Tab>
        ))}
      </TabsHeader>
      <TabsBody >
        {data.map(({ value, desc }) => (
          <TabPanel key={value} value={value} className="bg-white rounded-md mt-2">
            {desc}
          </TabPanel>
        ))}
      </TabsBody>
